...
class ChargeByCardCustomerOpenPay
{
    /**
     * Create Carge.
     *
     ...
     */
    public function __invoke($chargeData, $idcustomeropenpay)
    {
        $client = new Client(['http_errors' => false]);
        try {
            $response = $client->request(
                'POST',
                env('OPEN_PAY_ENVIROMENT').env('OPENPAY_ID').'/customers'.'/'.$idcustomeropenpay.'/charges', [
                'headers' => [
