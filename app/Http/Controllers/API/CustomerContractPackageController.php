...
use App\Http\Requests\Packages\ContractPackagesByCustomerRequest;
...
use App\Main\Subscription\Services\ContractPackagePaymentByCardCustomerOpenPayService;
use App\Utils\DateUtil;
use App\Utils\SendEmail;
use App\Utils\SMSUtil;

class CustomerContractPackageController extends Controller
{
     ...
     *                @OA\Items(type="string", example="El campo card id seleccionado no existe.")
     *            )
     *       )
     *     )
     *  ),
     ...
     *        @OA\Items(
     *            type="object",
     *            @OA\Property(property="cases_id", type="number", example="17", description="Id interno de la BD de ATA"),
     ...
     * )
     */
    public function index(ContractPackagesByCustomerRequest $request)
    {
        $user = $request->user();
        $nowDT = new \DateTime();
        try {
            ...
            // Search package for get plan´s id
            ...
            // Buscar la al cliente
            $customer = (new FindCustomerOpenPayDomain())(['id' => $user->id]);
            if (is_null($customer)) {
                throw new \Exception('El cliente no cuenta con tarjetas registradas', 404);
            }
            // Buscar la targeta
            $cards = (new FindCustomerCardOpenPayDomain())(['openpay_customer_cards.user_id' => $user->id, 'openpay_customer_cards.id' => $data['cardId']]);
            if (is_null($cards)) {
                throw new \Exception('Tarjeta no encontrada', 404);
            }
            $cardOpenpayId = $cards->id_card_open_pay;
            $idcustomeropenpay = $cards->idcustomeropenpay;

            $dt = (new \DateTime())->modify('-1 day');
            $trial_end_date = $dt->format('Y-m-d');

            // -- PROCESO DE SUSCRIPCIÓN OPEN PAY--
            $responseSubscriptionOpenPay = (new ContractPackagePaymentByCardCustomerOpenPayService())(
                $idcustomeropenpay,
                $cardOpenpayId,
                $planId,
                $trial_end_date
            ...
            $arrayResponseSubscription = json_decode($responseSubscriptionOpenPay['json'], true);
